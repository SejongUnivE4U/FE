...
const messages: Record<StateType, string> = {
  ...
  healthy: '훌륭해요!\n건강한 상태를 유지하고 있군요! 계속해서 관리해봅시다!', // 80~100
  moderate: '구강 상태가 보통이에요.\n조금 더 관심을 가져 보세요!', // 40~60
  good: '구강 상태가 양호합니다!\n지금 상태를 유지해보세요!', // 60~80
};
    ...
    const getUserData = async () => {
      try {
        ...
        if (userData.diagnoseNum === 0) {
          ...
          userData.lastDiagnoseScore > 40 &&
          userData.lastDiagnoseScore <= 60
        ) {
          setCharacterState('moderate');
        } else if (
          userData.lastDiagnoseScore > 60 &&
          userData.lastDiagnoseScore <= 80
        ) {
          setCharacterState('good');
        } else {
